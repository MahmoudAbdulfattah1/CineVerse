...
public class ContentRepositoryImpl implements ContentRepositoryCustom {
    ...
    public Page<Content> filterContent(List<String> genres, Integer year, Integer rate, ContentType contentType,
        ...
        List<Integer> contentIds = entityManager.createQuery(idQuery)
        ...
        contentQuery.where(contentRoot.get("id").in(contentIds));
        List<? extends Content> contentList = entityManager.createQuery(contentQuery).getResultList();
        ...
        return new PageImpl<>(new ArrayList<>(contentList), pageable, total);
    }
    ...
    public Page<Content> searchContent(String keyword, Pageable pageable) {
        ...
        List<Movie> movies = queryContent(Movie.class, keyword, pageSize);
        List<Series> series = queryContent(Series.class, keyword, pageSize);
        ...
        List<Content> pageContent = sortedContent.subList(start, end);
    ...
    private <T extends Content> List<T> queryContent(Class<T> entityClass, String keyword, int limit) {
        ...
        if (keyword != null && !keyword.isBlank()) {
            predicates.add(cb.like(cb.lower(idRoot.get("title")), "%" + keyword.toLowerCase() + "%"));
        }
        idQuery.where(cb.and(predicates.toArray(new Predicate[0])));

        List<Integer> ids = entityManager.createQuery(idQuery)
                .setMaxResults(limit)
                .getResultList();
    ...
    private List<Predicate> buildPredicates(CriteriaBuilder criteriaBuilder, Root<? extends Content> root,
                                            CriteriaQuery<?> query, List<String> genres, Integer year,
                                            Integer rate, ContentType contentType, String language) {
        List<Predicate> predicates = new ArrayList<>();
        ...
        if (genres != null && !genres.isEmpty()) {
            ...
            subquery.select(subRoot.get("content").get("id"))
                    ...
                    .having(criteriaBuilder.equal(criteriaBuilder.countDistinct(subGenreJoin.get("name")),
                            genres.size()));
